...
Func heat_step(Func input, Expr max_val) {
	...
	Halide::Var _x0, _x1, _x2, _x4, _x5;
	clamped_0.split(x, x, _x0, 32).reorder(_x0, x).vectorize(_x0, 8).compute_root();
	heat_0.split(x, x, _x1, 32).reorder(_x1, x).vectorize(_x1, 8).compute_root();
	clamped_1.split(x, x, _x2, 2).reorder(_x2, x).compute_root();
	heat_1.compute_root();
	clamped_2.split(x, x, _x4, 16).reorder(_x4, x).vectorize(_x4, 8).compute_root();
	heat_2.split(x, x, _x5, 4).reorder(_x5, x).compute_root();
