...
Func heat_step(Func input, Expr max_val) {
	...
	Halide::Var _x0, _x1, _x2, _x3, _x4, _x5;
	clamped_0.split(x, x, _x0, 2).reorder(_x0, x).compute_root();
	heat_0.split(x, x, _x1, 4).reorder(_x1, x).vectorize(_x1, 2).compute_at(
			clamped_1, x);
	clamped_1.split(x, x, _x2, 8).reorder(_x2, x).vectorize(_x2, 4).compute_at(
			heat_1, x);
	heat_1.split(x, x, _x3, 4).reorder(_x3, x).vectorize(_x3, 2).compute_root();
	clamped_2.split(x, x, _x4, 4).reorder(_x4, x).compute_root();
	heat_2.split(x, x, _x5, 16).reorder(_x5, x).vectorize(_x5, 4).compute_root();
	return heat_2;
