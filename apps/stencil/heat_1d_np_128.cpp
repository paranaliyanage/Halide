...
Func heat_step(Func input, Expr max_val) {
	...
	clamped_0.split(x, x, _x0, 64).reorder(_x0, x).vectorize(_x0, 8).compute_at(
			heat_0, x);
	heat_0.split(x, x, _x1, 8).reorder(_x1, x).vectorize(_x1, 2).compute_root();
	clamped_1.split(x, x, _x2, 8).reorder(_x2, x).compute_at(heat_1, x);
	heat_1.split(x, x, _x3, 8).reorder(_x3, x).vectorize(_x3, 2).compute_root();
	clamped_2.split(x, x, _x4, 8).reorder(_x4, x).vectorize(_x4, 4).compute_at(
			heat_2, x);
	heat_2.split(x, x, _x5, 4).reorder(_x5, x).vectorize(_x5, 2).compute_root();
