...
Func heat_step(Func input, Expr max_x, Expr max_y) {
	...
	Halide::Var _x0, _y1, _x2, _x4, _y5, _x6, _y7;
	clamped_0.split(x, x, _x0, 16).split(y, y, _y1, 2).reorder(_x0, x, _y1, y).reorder_storage(
			y, x).vectorize(_x0, 8).compute_at(heat_1, y);
	heat_0.split(x, x, _x2, 8).reorder(y, _x2, x).reorder_storage(y, x).vectorize(
			y, 2).compute_at(clamped_1, _y5);
	clamped_1.split(x, x, _x4, 2).split(y, y, _y5, 4).reorder(_y5, _x4, y, x).reorder_storage(
			x, y).compute_at(heat_1, _y7);
	heat_1.split(x, x, _x6, 8).split(y, y, _y7, 64).reorder(_y7, y, _x6, x).reorder_storage(
			x, y).vectorize(_y7, 8).compute_root();
	clamped_2.reorder(y, x).reorder_storage(x, y).vectorize(y, 4).compute_at(
			heat_2, x);
	heat_2.reorder(x, y).reorder_storage(x, y).vectorize(x, 8).parallel(y).compute_root();
