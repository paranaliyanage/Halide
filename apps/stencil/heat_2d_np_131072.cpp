...
Func heat_step(Func input, Expr max_x, Expr max_y) {
	...
	Halide::Var _x0, _y1, _x2, _y3, _x4, _y5, _x6, _y7, _y9, _x10, _y11;
	clamped_0.split(x, x, _x0, 4).split(y, y, _y1, 2).reorder(_x0, x, _y1, y).reorder_storage(
			y, x).compute_at(clamped_1, _x4);
	heat_0.split(x, x, _x2, 16).split(y, y, _y3, 4).reorder(_x2, _y3, x, y).reorder_storage(
			y, x).vectorize(_x2, 8).compute_at(clamped_1, _y5);
	clamped_1.split(x, x, _x4, 2).split(y, y, _y5, 4).reorder(_y5, _x4, y, x).reorder_storage(
			x, y).vectorize(_y5, 2).compute_at(heat_1, _y7);
	heat_1.split(x, x, _x6, 32).split(y, y, _y7, 4).reorder(_x6, _y7, y, x).reorder_storage(
			y, x).vectorize(_x6, 4).compute_root();
	clamped_2.split(y, y, _y9, 8).reorder(_y9, x, y).reorder_storage(x, y).compute_at(
			heat_2, x);
	heat_2.split(x, x, _x10, 32).split(y, y, _y11, 4).reorder(_x10, _y11, x, y).reorder_storage(
			x, y).vectorize(_x10, 4).parallel(y).compute_root();
