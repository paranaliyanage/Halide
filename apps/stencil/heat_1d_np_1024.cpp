...
Func heat_step(Func input, Expr max_val) {
	...
	Halide::Var _x1, _x2, _x3, _x5;
	clamped_0.compute_at(heat_0, x);
	heat_0.split(x, x, _x1, 16).reorder(_x1, x).vectorize(_x1, 8).compute_root();
	clamped_1.split(x, x, _x2, 64).reorder(_x2, x).vectorize(_x2, 8).compute_root();
	heat_1.split(x, x, _x3, 2).reorder(_x3, x).compute_root();
	clamped_2.vectorize(x, 4).compute_root();
	heat_2.split(x, x, _x5, 8).reorder(_x5, x).vectorize(_x5, 2).compute_root();
