...
Func heat_step(Func input, Expr max_val) {
	...
	Halide::Var _x0, _x2, _x3, _x5;
	clamped_0.split(x, x, _x0, 16).reorder(_x0, x).vectorize(_x0, 8).compute_root();
	heat_0.vectorize(x, 2).compute_at(clamped_1, x);
	clamped_1.split(x, x, _x2, 16).reorder(_x2, x).vectorize(_x2, 8).compute_root();
	heat_1.split(x, x, _x3, 16).reorder(_x3, x).vectorize(_x3, 4).compute_root();
	clamped_2.vectorize(x, 4).compute_at(heat_2, x);
