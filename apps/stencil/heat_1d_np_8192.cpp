...
Func heat_step(Func input, Expr max_val) {
	...
	Halide::Var _x1, _x2, _x3, _x4;
	clamped_0.compute_root();
	heat_0.split(x, x, _x1, 8).reorder(_x1, x).vectorize(_x1, 4).compute_at(
			clamped_1, x);
	clamped_1.split(x, x, _x2, 8).reorder(_x2, x).vectorize(_x2, 2).compute_at(
			heat_1, x);
