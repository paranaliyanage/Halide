...
Func heat_step(Func input, Expr max_x, Expr max_y) {
	...
	Halide::Var _x0, _y1, _x2, _y3, _x4, _y5, _x6, _y7, _x10;
	clamped_0.split(x, x, _x0, 4).split(y, y, _y1, 4).reorder(_x0, _y1, x, y).reorder_storage(
			x, y).vectorize(_x0, 2).compute_at(clamped_1, y);
	heat_0.split(x, x, _x2, 4).split(y, y, _y3, 4).reorder(_x2, _y3, x, y).reorder_storage(
			y, x).compute_at(clamped_1, _y5);
	clamped_1.split(x, x, _x4, 32).split(y, y, _y5, 4).reorder(_x4, _y5, y, x).reorder_storage(
			x, y).vectorize(_x4, 8).compute_root();
	heat_1.split(x, x, _x6, 4).split(y, y, _y7, 2).reorder(_y7, _x6, y, x).reorder_storage(
			x, y).compute_root();
