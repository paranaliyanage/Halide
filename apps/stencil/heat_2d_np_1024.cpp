...
Func heat_step(Func input, Expr max_x, Expr max_y) {
	...
	Halide::Var _x0, _y1, _x2, _y3, _x4, _y5, _y7, _x8, _y9, _y11;
	clamped_0.split(x, x, _x0, 32).split(y, y, _y1, 8).reorder(_x0, x, _y1, y).reorder_storage(
			x, y).vectorize(_x0, 4).compute_root();
	heat_0.split(x, x, _x2, 16).split(y, y, _y3, 8).reorder(_x2, x, _y3, y).reorder_storage(
			y, x).vectorize(_x2, 4).compute_at(clamped_1, _y5);
	clamped_1.split(x, x, _x4, 4).split(y, y, _y5, 16).reorder(_y5, _x4, y, x).reorder_storage(
			x, y).vectorize(_y5, 4).compute_at(heat_1, y);
	heat_1.split(y, y, _y7, 2).reorder(_y7, x, y).reorder_storage(y, x).parallel(
			y).compute_root();
	clamped_2.split(x, x, _x8, 2).split(y, y, _y9, 16).reorder(_y9, _x8, y, x).reorder_storage(
			x, y).vectorize(_y9, 4).compute_at(heat_2, y);
	heat_2.split(y, y, _y11, 16).reorder(_y11, x, y).reorder_storage(y, x).vectorize(
			_y11, 2).parallel(y).compute_root();
