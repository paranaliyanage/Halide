...
Func heat_step(Func input, Expr max_x, Expr max_y) {
	...
	Halide::Var _x0, _y1, _x6, _x10, _y11;
	clamped_0.split(x, x, _x0, 8).split(y, y, _y1, 64).reorder(_y1, _x0, y, x).reorder_storage(
			x, y).vectorize(_y1, 8).parallel(x).compute_root();
	heat_0.reorder(y, x).reorder_storage(x, y).vectorize(y, 2).compute_at(
			clamped_1, x);
	clamped_1.reorder(y, x).reorder_storage(x, y).vectorize(y, 2).compute_root();
	heat_1.split(x, x, _x6, 8).reorder(y, _x6, x).reorder_storage(x, y).compute_at(
			heat_2, y);
	clamped_2.reorder(x, y).reorder_storage(x, y).vectorize(x, 8).compute_at(
			heat_2, _y11);
