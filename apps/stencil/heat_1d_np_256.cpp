...
Func heat_step(Func input, Expr max_val) {
	...
	Halide::Var _x2, _x3, _x4, _x5;
	clamped_0.vectorize(x, 8).compute_at(heat_0, x);
	heat_0.compute_at(heat_1, x);
	clamped_1.split(x, x, _x2, 32).reorder(_x2, x).vectorize(_x2, 4).compute_at(
			heat_1, _x3);
	heat_1.split(x, x, _x3, 32).reorder(_x3, x).vectorize(_x3, 8).compute_root();
	clamped_2.split(x, x, _x4, 16).reorder(_x4, x).vectorize(_x4, 4).compute_at(
			heat_2, x);
