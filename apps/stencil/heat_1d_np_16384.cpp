...
Func heat_step(Func input, Expr max_val) {
	...
	Halide::Var _x0, _x1, _x2, _x4;
	clamped_0.split(x, x, _x0, 4).reorder(_x0, x).compute_at(heat_0, x);
	heat_0.split(x, x, _x1, 16).reorder(_x1, x).vectorize(_x1, 4).compute_at(
			clamped_1, x);
	clamped_1.split(x, x, _x2, 8).reorder(_x2, x).compute_root();
	heat_1.vectorize(x, 2).compute_root();
	clamped_2.split(x, x, _x4, 8).reorder(_x4, x).vectorize(_x4, 4).compute_root();
	heat_2.vectorize(x, 8).compute_root();
