...
Func heat_step(Func input, Expr max_val) {
	...
	clamped_0.split(x, x, _x0, 8).reorder(_x0, x).compute_at(heat_0, _x1);
	heat_0.split(x, x, _x1, 64).reorder(_x1, x).vectorize(_x1, 8).compute_root();
	clamped_1.split(x, x, _x2, 4).reorder(_x2, x).vectorize(_x2, 2).compute_at(
			heat_1, x);
