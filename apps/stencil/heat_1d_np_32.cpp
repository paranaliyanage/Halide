...
Func heat_step(Func input, Expr max_val) {
	...
	Halide::Var _x1, _x2, _x3, _x4, _x5;
	clamped_0.vectorize(x, 4).compute_at(heat_0, _x1);
	heat_0.split(x, x, _x1, 32).reorder(_x1, x).vectorize(_x1, 8).compute_root();
	clamped_1.split(x, x, _x2, 32).reorder(_x2, x).vectorize(_x2, 4).compute_at(
			heat_1, _x3);
	heat_1.split(x, x, _x3, 4).reorder(_x3, x).vectorize(_x3, 2).compute_root();
	clamped_2.split(x, x, _x4, 32).reorder(_x4, x).vectorize(_x4, 8).compute_at(
			heat_2, _x5);
	heat_2.split(x, x, _x5, 32).reorder(_x5, x).vectorize(_x5, 4).compute_root();
